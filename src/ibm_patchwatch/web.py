...
function compare(id,i,e){if(e.support_status==="not_supported")return "NOT_SUPPORTED";const a=e.available;if(!a||!a.version)return "CHECK_REQUIRED";
 if(["content_manager","websphere","ibm_java"].includes(id)){const state=numeric(i,a);return id!=="ibm_java"&&state==="CURRENT"?"CHECK_REQUIRED":state;}
 ...
 if(id==="iccsap"){const v=cmp(i.version,a.version);if(v===null)return "CHECK_REQUIRED";if(v<0)return "UPDATE_AVAILABLE";if(v>0)return "NEWER_THAN_CATALOG";const j=cmp(i.jre_version,a.jre_version);return j!==null&&j<0?"UPDATE_AVAILABLE":"CHECK_REQUIRED"}
...
function available(id,e){const a=e&&e.available||{};return [a.version,a.interim_fix!=null?"iFix "+a.interim_fix:null,a.special_build,a.jre_version?"JRE "+a.jre_version:null,...((e&&e.interim_fixes)||[]).map(f=>f.fix_id+" (non-cumulative; base "+((f.applies_to||{}).min||"?")+"–"+((f.applies_to||{}).max||"?")+")")].filter(Boolean).join(" ")||"?"}
